    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case 6: {
                ...
                cin >> filename;
                storeData(filename, point2DList, point3DList, line2DList, line3DList, currentFilter);
...
void viewData(const vector<Point2D> &point2DList, 
	...
	if (currentFilter == "Point2D") {
        ...
        // Remove duplicates for Point3D
        vector<Point3D> uniquePoint3DList = point3DList;
        sort(uniquePoint3DList.begin(), uniquePoint3DList.end());
        uniquePoint3DList.erase(unique(uniquePoint3DList.begin(), uniquePoint3DList.end()), uniquePoint3DList.end());


		vector<Point3D> sortedList = uniquePoint3DList;
        ...
        if (currentCriteria == "x-ordinate") {
			...
			sort(sortedList.begin(), sortedList.end(), [](const Point3D &a, const Point3D &b) {
				if (a.getY() != b.getY()) return a.getY() < b.getY();  // ✅ Primary: Sort by Y
				if (a.getX() != b.getX()) return a.getX() < b.getX();  // ✅ Secondary: Sort by X
				return a.getZ() < b.getZ();  // ✅ Tertiary: Sort by Z
			});
		} 
		else if (currentCriteria == "z-ordinate") {
			sort(sortedList.begin(), sortedList.end(), [](const Point3D &a, const Point3D &b) {
				if (a.getZ() != b.getZ()) return a.getZ() < b.getZ();  // ✅ Primary: Sort by Z
				if (a.getX() != b.getX()) return a.getX() < b.getX();  // ✅ Secondary: Sort by X
				return a.getY() < b.getY();  // ✅ Tertiary: Sort by Y
			});
		} 
        ...
        cout << "- - - - - - - - - - - - - - - - - - -" << endl;
        for (const auto &point : sortedList) {
            cout << "[" << setw(4) << point.getX() << ", " << setw(4) << point.getY() << ", " << setw(4) << point.getZ() << "]"
                 << setw(10) << fixed << setprecision(3) << point.getScalarValue() << endl;
        }
...
void storeData(const string &filename, const vector<Point2D> &point2DList, const vector<Point3D> &point3DList,
               const vector<Line2D> &line2DList, const vector<Line3D> &line3DList, const string &currentFilter) {
    ...
    if (currentFilter == "Point2D") {
        ...
        for (const auto &point : point2DList) {
            file << setw(4) << point.getX() << setw(4) << point.getY()
                 << setw(15) << fixed << setprecision(3) << point.getScalarValue() << endl;
            recordCount++;
        }
    } else if (currentFilter == "Point3D") {
        // Remove duplicates for Point3D
        set<Point3D> uniquePoint3DList(point3DList.begin(), point3DList.end());
        vector<Point3D> sortedList(uniquePoint3DList.begin(), uniquePoint3DList.end());
        ...
        file << "Pt1(X, Y)    Pt2(X, Y)    Length\n";
        file << "-------------------------------\n";
        for (const auto &line : line2DList) {
            file << "(" << setw(4) << line.getPt1().getX() << ", " << setw(4) << line.getPt1().getY() << ")"
                 << "  (" << setw(4) << line.getPt2().getX() << ", " << setw(4) << line.getPt2().getY() << ")"
                 << setw(12) << fixed << setprecision(3) << line.getScalarValue() << endl;
            recordCount++;
        }
    } else if (currentFilter == "Line3D") {
        file << "Pt1(X, Y, Z)    Pt2(X, Y, Z)    Length\n";
        file << "--------------------------------------\n";
        ...
        file << "No data to save for the current filter." << endl;
    }
