//File: A1_c.java
// RandomSet generator is in the Set, but not in Array S
// set have unique values, not reapeted and is in string.
...
class Set{
    ...
    private ArrayList<NumberType> contents;
    ...
    public Set() {
        ...
        this.randomSet = generateRandomSet(10); // Generate the random set inside the default constructor

        // Add the elements from randomSet to this.s
        for (NumberType element : this.randomSet) {
            this.s.add(element);
        }
    ...
    private NumberType[] generateRandomSet(int maxSize){
        ...
        if (setSize > numValues) {
             setSize = numValues;  //E.g: if setSize= 4, numValue=11. prevent setSize going above numValues
        }
            ...
            for (int i = 0; i < setSize; i++) {
                ...
                } while (usedIndices[randomIndex]); // check if the element already exist in arraylist
    ...
    public void printAndAddRandomSet() {
        for (int i = 0; i < randomSet.length; i++) {
            System.out.print(randomSet[i].name());
            if (i < randomSet.length - 1) {
                System.out.print(", ");
            }
            addNumberTypeElement(randomSet[i]); // Add the current element to yourSet
        }
    }
